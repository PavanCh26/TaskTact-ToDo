...
def add_task(tasks):
    ...
    if due_date:
        try:
            ...
            # <-- stricter: due must be AFTER today (not today or past)
            if due <= today:
                print("⚠️  Due date must be after today! Please enter a future date.")
                return
        except ValueError:
        ...
        if 0 <= num < len(tasks):
            ...
            if new_due:
                try:
                    ...
                    today = datetime.date.today()
                    # <-- stricter: reject today or past
                    if due <= today:
                        print("⚠️  Due date must be after today! Keeping old date.")
                    else:
